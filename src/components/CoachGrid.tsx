import type { CoachGuessRow, CellStatus } from '../types/database';
import { ArrowUp, ArrowDown } from 'lucide-react';

interface CoachGridProps {
  guesses: CoachGuessRow[];
...
const COLUMNS = [
  { key: 'name', label: 'Coach', width: '28%' },
  { key: 'nationality', label: 'Nation', width: '18%' },
  { key: 'club', label: 'Club', width: '18%' },
  { key: 'league', label: 'League', width: '18%' },
  { key: 'age', label: 'Age', width: '18%' },
] as const;

const CELL_TRUNCATE = 'overflow-hidden text-ellipsis whitespace-nowrap';

export function CoachGrid({ guesses, maxGuesses }: CoachGridProps) {
  return (
          ...
          {guesses.map((row, idx) => (
            <tr key={idx} className="animate-row-in">
              {COLUMNS.map((col) => {
                ...
                const showArrow = col.key === 'age';
                const fullText = col.key === 'name'
                  ? `${cell.value} (${row.coach.club})`
                  : cell.value;
                  ...
                  >
                    {col.key === 'name' ? (
                      <div className="flex flex-col items-start leading-tight">
                        ...
                        <span className="text-xs text-slate-500 overflow-hidden text-ellipsis whitespace-nowrap max-w-full">{row.coach.club}</span>
                      </div>
                    ) : (
