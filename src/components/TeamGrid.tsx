import type { TeamGuessRow, CellStatus } from '../types/database';
import { ArrowUp, ArrowDown } from 'lucide-react';

interface TeamGridProps {
  guesses: TeamGuessRow[];
...
const COLUMNS = [
  { key: 'name', label: 'Team', width: '20%' },
  { key: 'league', label: 'League', width: '16%' },
  { key: 'country', label: 'Country', width: '14%' },
  { key: 'overall', label: 'OVR', width: '10%' },
  { key: 'stadium', label: 'Stadium', width: '18%' },
  { key: 'defStyle', label: 'DEF', width: '11%' },
  { key: 'offStyle', label: 'OFF', width: '11%' },
] as const;

const CELL_TRUNCATE = 'overflow-hidden text-ellipsis whitespace-nowrap';

export function TeamGrid({ guesses, maxGuesses }: TeamGridProps) {
  return (
          ...
          {guesses.map((row, idx) => (
            <tr key={idx} className="animate-row-in">
              {COLUMNS.map((col) => {
                ...
                const showArrow = col.key === 'overall';
                const fullText = col.key === 'name'
                  ? `${cell.value} (${row.team.league})`
                  : cell.value;
                  ...
                  >
                    {col.key === 'name' ? (
                      <div className="flex flex-col items-start leading-tight">
                        ...
                        <span className="text-xs text-slate-500 overflow-hidden text-ellipsis whitespace-nowrap max-w-full">{row.team.league}</span>
                      </div>
                    ) : (
