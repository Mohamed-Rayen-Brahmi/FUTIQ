...
interface CoachSearchProps {
  onGuess: (coach: Coach) => void;
  disabled: boolean;
  guessedNames: Set<string>;
}

export function CoachSearch({ onGuess, disabled, guessedNames }: CoachSearchProps) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const { data, error } = await supabase
          .rpc('search_coaches', { search_query: query.trim() });
  ...
  const handleSelect = useCallback((coach: Coach) => {
    onGuess(coach);
    setQuery('');
    setResults([]);
    setOpen(false);
    ...
    <div ref={containerRef} className="relative w-full max-w-md mx-auto">
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 mt-1 w-full bg-ink-panel border border-ink-border rounded-lg overflow-hidden shadow-xl max-h-72 overflow-y-auto">
          {results.map((coach) => {
            const alreadyGuessed = guessedNames.has(coach.name.toLowerCase());
            return (
              <button
                key={coach.id}
                onClick={() => !alreadyGuessed && handleSelect(coach)}
              ...
              >
                <div className="font-body text-slate-200">{coach.name}</div>
