...
interface TeamSearchProps {
  onGuess: (team: Team) => void;
  disabled: boolean;
  guessedNames: Set<string>;
}

export function TeamSearch({ onGuess, disabled, guessedNames }: TeamSearchProps) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const { data, error } = await supabase
          .rpc('search_teams', { search_query: query.trim() });
    ...
    <div ref={containerRef} className="relative w-full max-w-md mx-auto">
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 mt-1 w-full bg-ink-panel border border-ink-border rounded-lg overflow-hidden shadow-xl max-h-72 overflow-y-auto">
          {results.map((team) => {
            const alreadyGuessed = guessedNames.has(team.name.toLowerCase());
            return (
              <button
                key={team.id}
                onClick={() => !alreadyGuessed && handleSelect(team)}
              ...
              >
                <div className="font-body text-slate-200">{team.name}</div>
